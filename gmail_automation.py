...
class GmailAutomation:
    def __init__(self):
        ...
        self.mail_sender_agent = Agent(
            ...
            backstory="You are an automated Gmail agent. You send interview emails to candidates having the email address {receiver_email} based on the markdown input provided.",
            llm=llm,
            tools=[gmail_tool],
            allow_delegation=False,
            verbose=True
    ...
    def create_task(self, job_description, candidate_name, organization_name, interview_time, receiver_email,google_form):
        # Email Composition Task
        
        self.mail_composer_task = Task(
        name="mail_composer_task",
        description=(
        f"You are reaching out to a potential candidate named **{candidate_name}** who has not applied directly, "
    ...
    agent=self.mail_composer_agent
        ...
        self.mail_sender_task = Task(
            description="Send the composed interview invitation email to the candidate having the email address {receiver_email} using GmailTool. Use the markdown output as the email body.",
    ...
    def kickoff(self, job_description, candidate_name, organization_name, interview_time, receiver_email,google_form):
        self.mail_composer_task, self.mail_sender_task = self.create_task(
            job_description, candidate_name, organization_name, interview_time, receiver_email,google_form
        )
        crew = Crew(
...
if __name__ == '__main__':
    ...
    candidate_name = "Nishaad Mishra"
    organization_name = "OpenAI Talent Team"
    interview_time = "Monday, April 7th at 11:00 AM IST"
    candidate_email = "nishaad.may.27@gmail.com"
    google_form = "https://forms.gle/exampleform"

    gmail_automation = GmailAutomation()
    result = gmail_automation.kickoff(job_description, candidate_name, organization_name, interview_time, candidate_email,google_form)
    print(result)
